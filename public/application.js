...
var hidePreLogin = function hidePreLogin() {
  $('#pre-login').slideUp('slow');
};

var showPreLogin = function showPreLogin() {
  $('#pre-login').slideDown('slow');
};

var hidePostLogin = function hidePostLogin() {
  $('#post-login').slideUp('slow');
};

var showPostLogin = function showPostLogin() {
  $('#post-login').slideDown('slow');
};

// Password functions
var showChangePasswordButton = function showChangePasswordButton() {
...
var onStartNewGameSuccess = function onStartNewGameSuccess(data) {
...
var onStartNewGameFailure = function onStartNewGameFailure() {
  $('#message').text('Error encountered starting a new game.  Please try again.');
};

var onGetOpenGames = function onGetOpenGames(data) {
  store.history = data;
  $('#get-open-gms-msg').text('');
  var game = store.history.games.length;
...
module.exports = {
  ...
  showChangePasswordButton: showChangePasswordButton,
  ...
  onGetOpenGames: onGetOpenGames,
  onStartNewGameSuccess: onStartNewGameSuccess,
  onStartNewGameFailure: onStartNewGameFailure,
  hidePreLogin: hidePreLogin,
  showPreLogin: showPreLogin,
  hidePostLogin: hidePostLogin,
...
// Sign up functions
var onSignUp = function onSignUp(data) {
  return $.ajax({
...
// Game functions
var onStartNewGame = function onStartNewGame(token) {
...
var prepAPIData = function prepAPIData() {
  var index = 0;
  var player = store.game.cells[0];
  var data = {
    'game': {
      'cell': {
        'index': index,
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var userForm = getFormFields(this);
  if (userForm.credentials.email === '' && userForm.credentials.password === '') {
    ...
    api.onSignIn(userForm).then(ui.onSignInSuccess).catch(ui.onSignInFailure);
  }
};

var onAcctExists = function onAcctExists(event) {
  event.preventDefault();
  $('#sign-up-form')[0].reset();
...
var onSignUp = function onSignUp(event) {
  ...
  if (userForm.credentials.password !== userForm.credentials.password_confirmation) {
    ...
    api.onSignUp(userForm).then(ui.onSignUpSuccess).catch(ui.onSignUpFailure);
  }
};

var onAcctNew = function onAcctNew(event) {
  event.preventDefault();
...
var onSignOut = function onSignOut(event) {
  event.preventDefault();
  api.onSignOut().then(ui.onSignOutSuccess);
};
...
var onChangePassword = function onChangePassword(event) {
  ...
  if (userForm.passwords.old === '' && userForm.passwords.new === '') {
    ...
    api.onChangePassword(userForm).then(ui.onChangePasswordSuccess).catch(ui.onChangePasswordFailure);
  }
};

var onCancelChangePassword = function onCancelChangePassword() {
  event.preventDefault();
...
var onStartNewGame = function onStartNewGame(event) {
  ...
  api.onStartNewGame(store.user.token).then(ui.onStartNewGameSuccess).then(engine.setBoard).catch(ui.onStartNewGameFailure);
};

var onGetOpenGames = function onGetOpenGames(event) {
  event.preventDefault();
  console.log('events.onGetOpenGames invoked');
  api.onGetOpenGames().then(ui.onGetOpenGames);
};

module.exports = {
  onAcctNew: onAcctNew,
  onAcctExists: onAcctExists,
  onSignUp: onSignUp,
  onSignIn: onSignIn,
  onSignOut: onSignOut,
  onChangePassword: onChangePassword,
...
};var checkForWin = function checkForWin(index, player) {
  checkDiagonal();
  checkHorizontalGrid();
  checkVerticalGrid();
  if (moveCount > 8) {
...
var checkDiagonal = function checkDiagonal() {
  ...
  if (store.game.cells[6] === 'X' && store.game.cells[4] === 'X' && store.game.cells[2] === 'X') {
    ui.declareWinner('X');
    moveCount = 0;
    var _data = api.prepAPIData();
    api.updateGameAPI(_data);
  }
  if (store.game.cells[0] === 'O' && store.game.cells[4] === 'O' && store.game.cells[8] === 'O') {
    ui.declareWinner('O');
    moveCount = 0;
    var _data2 = api.prepAPIData();
    api.updateGameAPI(_data2);
  }
  if (store.game.cells[6] === 'O' && store.game.cells[4] === 'O' && store.game.cells[2] === 'O') {
    ui.declareWinner('O');
    moveCount = 0;
    var _data3 = api.prepAPIData();
    api.updateGameAPI(_data3);
  }
};

// Check the rows for win by X or O
var checkHorizontalGrid = function checkHorizontalGrid() {
  var row = '';
  for (var i = 0; i < 9; i += 3) {
    for (var a = i; a < i + 3; a++) {
    ...
    if (row === 'XXX') {
      ...
      ui.declareWinner('O');
      moveCount = 0;
      var _data4 = api.prepAPIData();
      api.updateGameAPI(_data4);
      row = '';
      return;
    } else {
      row = '';
...
var checkVerticalGrid = function checkVerticalGrid() {
  var row = '';
  for (var i = 0; i < 9; i++) {
    for (var a = i; a < i + 7; a += 3) {
      row += store.game.cells[a];
    }
    if (row === 'XXX') {
      ui.declareWinner('X');
      ...
      ui.declareWinner('O');
      moveCount = 0;
      var _data5 = api.prepAPIData();
      api.updateGameAPI(_data5);
      row = '';
      return;
    } else {
      row = '';
...
var playerMove = function playerMove(gridPos) {
  var index = gameObject[gridPos].arrayIndex;
  if (store.game.cells[index] === '') {
    if (isEven(moveCount)) {
      ...
      moveCount++;
      api.updateGameAPI(data).then($('#' + gridPos).text(player));
    } else {
      var _player = 'O';
      if (store.game.cells[index] === '') {
        store.game.cells[index] = _player;
        var _data6 = {
          'game': {
            'cell': {
              'index': index,
        ...
        moveCount++;
        api.updateGameAPI(_data6);
        $('#' + gridPos).text(_player);
      }
    }
  } else {
...
var recordMove = function recordMove(index, player, gridPos) {
  ...
  if (store.game.cells[index] === '') {
    ...
    // Will only execute if grid position is anything but "!"
...
module.exports = {
  ...
  checkVerticalGrid: checkVerticalGrid,
  checkHorizontalGrid: checkHorizontalGrid,
